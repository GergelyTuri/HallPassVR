function [lickByLap] = vrLickByLap(vrBehStruc)
...
lickByLap = zeros(length(lapTime)+1,length(edges)-1); %nbins);
...
figure; imagesc(lickByLap);colormap('jet'); caxis([0 10]); title(filename); xlabel('position'); ylabel('lap #');
