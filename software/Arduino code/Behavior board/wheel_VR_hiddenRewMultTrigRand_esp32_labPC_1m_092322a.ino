...
String programName = "wheel_VR_hiddenRewMultTrigRand_esp32_1m_091722c";
...
int lickThresh = 4;//50; //150; // 5*touch thresh value (for 5 read sum)

// reward vars
int numRew = 4; //8; // number of reward zones (if isRandRew=0, make equal to length rewPosArr) 
int rewWidth = 200;//450;  // width of reward zone
int isRandRew = 1;
int rewPosArr[] = {50, 250, 500, 750, 1000, 1250, 1500, 1750}; // {100, 500, 1000, 1500, 2000, 2500, 3000, 3500}; // specify exact positions if isRandRew=0
...
int rewPosArrMaster[] = {50, 250, 500, 750, 1000, 1250, 1500, 1750}; //{100, 500, 1000, 1500, 2000, 2500, 3000, 3500}; //, 3600}; // possible start positions of reward zones
...
void checkLicks() {
  ...
  if (isESP == 1) { // if using ESP32 touch sensor pin
    ...
    touchVal = 60-touchRead(touchPin); // real read, inverted
  }
  else {
