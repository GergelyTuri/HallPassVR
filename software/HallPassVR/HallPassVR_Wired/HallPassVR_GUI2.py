...
# --- DEFINE ---
root = Tk()
root.geometry("1600x600+100+100") # set size of window ("1500x600+100+100") dim+offsetXY
...
ListFrame         = LabelFrame(root, text="1a. Build pattern (select/Add FOUR elements/jpgs per pattern I-III, then press GENERATE for each)", padx = 10,       pady= 10,) # Frame(root)
CombinationFrame  = LabelFrame(root, text="2. Pattern generated (and Path when all 3 patterns generated)", padx = 10,       pady= 10,) # Frame(root)
HistoryComb       = LabelFrame(root, text="OR 1b. Upload prev pattern (then press Generate on 1a.)", padx = 10,       pady= 10,) # Frame(root)
Input             = LabelFrame(root, text="3. Select Env. Params, name path, set track length, then press START", padx = 10,       pady= 10,) # Frame(root)
...
def ONCLICK_START():
    ...
    if ((INPUT_READY) and (PATH_READY)):
        ...
        os.system('python3 VR_OPEN.py') # this runs the VR_OPEN.py script that then calls main script and prints to TK window screen #2
...
Label5.grid(row = 3, column = 0, padx = 10, pady = 10, sticky = W) # (row = 3, column = 1, padx = 10, pady = 10, sticky = W)
