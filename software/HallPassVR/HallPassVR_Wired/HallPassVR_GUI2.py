...
root.geometry(
    "1600x600+100+100"
)  # set size of window ("1500x600+100+100") dim+offsetXY
root.title("HallPassVR GUI")

Path = ["Combination1.jpg", "Combination2.jpg", "Combination3.jpg"]

# FRAMES for the main widgets (see main frame params ~ln.1330)
ListFrame = LabelFrame(
    root,
    text="1a. Build pattern (select/Add FOUR elements/jpgs per pattern I-III, then press GENERATE for each)",
    padx=10,
    pady=10,
)  # Frame(root)
CombinationFrame = LabelFrame(
    root,
    text="2. Pattern generated (and Path when all 3 patterns generated)",
    padx=10,
    pady=10,
)  # Frame(root)
HistoryComb = LabelFrame(
    root,
    text="OR 1b. Upload prev pattern (then press Generate on 1a.)",
    padx=10,
    pady=10,
)  # Frame(root)
Input = LabelFrame(
    root,
    text="3. Select Env. Params, name path, set track length, then press START",
    padx=10,
    pady=10,
)  # Frame(root)
...
def INIT():
    ...
    ELEMENT_CONVERT_TABLE = {
        "NAME": [],
        "DECODE_NAME": [],
    }  # directory store iofrmation about inital name and name after decode
    PATTERN_DICT = {}
    PATH_DICT = {}
    ...
    DROPBOX_PATTERN_OLD = [
        ...
        0,
        0,
    ]  # Based on the fact: Pattern I is the default on DROPBOX_CLICKED
...
def JSON_INIT():
    ...
    json_string = json.dumps(
        PATTERN_DICT, indent=4
    )  # PATTERN_DICT can only be used when process data
...
def PATTERN_GEN(CURRSEL_ELEMENT_GEN):
    ...
    COMB_IMAGE_DECODE_NAME_LIST = [
        ELEMENT_CONVERT_TABLE["DECODE_NAME"][x] for x in CURRSEL_ELEMENT_GEN
    ]

    try:
        img_list = [
            ADDS_ELEMENT + ELEMENT_CONVERT_TABLE["NAME"][x] for x in CURRSEL_ELEMENT_GEN
        ]
        COMB_IMAGE_NAME_LIST = [
            Image.open(y)
            for y in [
                ADDS_ELEMENT + ELEMENT_CONVERT_TABLE["NAME"][x]
                for x in CURRSEL_ELEMENT_GEN
            ]
        ]
    except:
...
def CURRSEL_ELEMENT(self):
    ...
    if index1:
        ...
        selected_indices1_path = (
            ADDS_ELEMENT + ELEMENT_CONVERT_TABLE["NAME"][selected_indices1]
        )
        ...
        HIST_PATT_NAME_SEL_AB = LISTBOX_HIST_PATTERN.get(
            LISTBOX_HIST_PATTERN.curselection()
        )
...
def ONCLICK_ADD():  # Edit Need
    INDEX = 1
    try:
        ADD_ELEMENT_NUM = LISTBOX_ELEMENT.curselection()[
            0
        ]  # store information about pic in number

        if len(SEF_ELEMENT_G) < ONCADD_ELEMENT_G_LIM:  # ONCADD_ELEMENT_G_LIM = 4
            SEF_ELEMENT_G.append(ADD_ELEMENT_NUM)

            SEF_CHECK_DECODE_G.append(
                ELEMENT_CONVERT_TABLE["DECODE_NAME"][ADD_ELEMENT_NUM]
            )
...
def ONCLICK_GENERATE():
    ...
    if len(SEF_ELEMENT_G) < ONCADD_ELEMENT_G_LIM:
        print(
            "ERROR: ",
            "Need " + str(ONCADD_ELEMENT_G_LIM - len(SEF_ELEMENT_G)) + " ELEMENTS",
        )
        ...
        JSON_PROCESS(
            GEN_PATTERN_NAME, SEF_CHECK_DECODE_G, SEF_CHECK_NAME_G, SEF_ELEMENT_G
        )
...
def ONCLICK_UPLOAD():
    ...
    try:
        ...
        HIST_PATT_NAME_SEL = LISTBOX_HIST_PATTERN.get(
            LISTBOX_HIST_PATTERN.curselection()
        )
        ...
        try:
            with open("image/HIST_PATTERN/PATTERN.json", "r") as f:
                ...
                if HIST_PATT_NAME_AB in PATTERN_DICT_HIST:
                    ...
                    SEF_CHECK_DECODE_G = PATTERN_DICT_HIST[HIST_PATT_NAME_AB][
                        "ELEMENT_DECODE_INDEX"
                    ]
                    SEF_CHECK_NAME_G = PATTERN_DICT_HIST[HIST_PATT_NAME_AB][
                        "ELEMENT_NAME_IMG"
                    ]
                    SEF_ELEMENT_G = PATTERN_DICT_HIST[HIST_PATT_NAME_AB][
                        "ELEMENT_NUM_DIG"
                    ]
...
def ONCLICK_START():
    ...
    if (INPUT_READY) and (PATH_READY):
        ...
        os.system(
            "python3 VR_OPEN.py"
        )  # this runs the VR_OPEN.py script that then calls main script and prints to TK window screen #2

    else:
        print("INPUT INFORMATION MISSING, VR CAN'T BE GENERATED")
...
def INPUT():
    ...
    try:
        if (
            (INPUT_FLOOR_ELEMENT != "")
            and (INPUT_CEILING_ELEMENT != "")
            and (INPUT_EXPT_NAME != "")
            and (int(INPUT_EXPT_LEN) > 0)
        ):
            INPUT_READY = 1
...
def IMG_RESIZE_CANVAS(img, size, canvas):
    ...
    IMG_VIEW_PATTERN = canvas.create_image(0, 0, image=IMG_PATH_NEW, anchor="nw")
...
def SELF_CHECK(INDEX):  # Edit Need
    ...
    if len(SEF_ELEMENT_G) == 0:
        ...
        for element in range(len(SEF_ELEMENT_G)):
            if (
                ELEMENT_CONVERT_TABLE["DECODE_NAME"][SEF_ELEMENT_G[element]]
                == SEF_CHECK_DECODE_G[element]
            ):
                ...
                if len_json == 1:
                    if "format" not in HIST_PATT:
                        ...
                        if len_img == len_json - 1:
                            ...
                            print(
                                "ERROR: SIZE of the IMG folder not match with JSON, PROGRAM CLOSE in 3S"
                            )
                            time.sleep(3)
                            quit()
                else:
                    ...
                    if len_img == len_json - 1:
                        ...
                        img_absolute_list = [
                            ...
                            for element in fileName_absolute_list
                        ]
                        ...
                        if error_count != 0:
                            print(
                                "Special case: self check JSON vs .jpg File Error Count: ",
                                error_count,
                            )
                            time.sleep(3)
                            quit()
                        ...
                        if error_count:
                            print(
                                "ERROR exists when reading JSON or .jpg file, PROGRAM CLOSE in 3S"
                            )
                            time.sleep(3)
                            quit()
                    else:
    ...
    if (INDEX == 8) or (INDEX == 0):
        ...
        if len_json == 1:
            ...
            if len_img == len_json - 1:
                fileName_absolute_list = [os.path.basename(s) for s in filelist]
                img_absolute_list = [
                    element[: element.index(".")] for element in fileName_absolute_list
                ]
...
CANVAS_ELEMENT.grid(row=5, column=5, columnspan=1, sticky=W)
...
Button_Delete = Button(
    ListFrame, text="Delete", width=5, height=2, command=ONCLICK_DELETE
)
Button_Delete_All = Button(
    ListFrame, text="Delete All", width=7, height=2, command=ONCLICK_DELETE_ALL
)
Button_Generate = Button(
    ListFrame, text="Generate", width=8, height=2, command=ONCLICK_GENERATE
)
Button_Exit = Button(root, text="Exit GUI", width=10, height=2, command=ONCLICK_EXIT)
Button_Upload = Button(
    HistoryComb, text="Upload", width=7, height=2, command=ONCLICK_UPLOAD
)
Button_Delete_hi = Button(
    HistoryComb, text="Delete", width=7, height=2, command=ONCLICK_HIS_PATTERN_DELETE
)
Button_Del_hi_all = Button(
    ...
    width=10,
    height=2,
    command=ONCLICK_HIS_PATTERN_DELETEALL,
)
# Button_Input      = Button(Input,       text = "Read",       width = 7,  height = 2, command= ONCLICK_INPUT_READ)
Button_Start = Button(root, text="Start VR", width=10, height=2, command=ONCLICK_START)
...
Button_Exit.grid(row=10, column=12, sticky=N)  # (row = 10, column = 12,  sticky = N)
Button_Start.grid(row=10, column=11, sticky=N)  # (row = 10, column = 11,  sticky = N)
...
# --- Frame GRID --- (these are the basic GUI frame coord for the diff widgets)
ListFrame.grid(row=0, column=0, columnspan=10, padx=10, pady=10, sticky=W)
CombinationFrame.grid(row=0, column=10, padx=20, pady=20, sticky=W)
HistoryComb.grid(row=8, column=0, rowspan=5, columnspan=8, padx=10, pady=10, sticky=W)
Input.grid(row=8, column=7, columnspan=5, sticky=W)

# --- LABEL --- (NOTE: grid coord are with respect to widget frame grid coord above)
...
Label5 = Label(HistoryComb, text="Pattern selected", font=("Arial", 12))
...
# Label10 = Label(Input,            text =" Elements",              font = ("Arial", 12))
...
# Label10.grid(row = 0, column = 0, padx = 10, pady = 10, sticky = N)


# --- CANVAS --- For drawing the pattern rectangles
