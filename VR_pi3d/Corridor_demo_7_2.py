...
DISPLAY = pi3d.Display.create(w=winw, h=winh - bord, far=4000.0,background=(0, 0, 0, 0), frames_per_second=300)
...
def pattern_get():
  ...
  pattern_list  = sorted(pattern_list, key = unicode.lower)
...
#sorted_file  = sorted(file_list, key = str.lower)
...
initialxm = -20 #center x position - half of the length +1 this case: 150 - 150/2 +1, when changing to serial input position, data should be changed to (pos+138)
initialym = avhgt/2
initialzm = 9
...
while DISPLAY.loop_running():
	...
	while ardSer.in_waiting:
		...
		try:
			...
			if ardStr.find(',')!=-1:
				print("inside")
				dy_string = ardStr[0:ardStr.find(',')]
				print(dy_string)
				e_dy_string = [int(element) for element in dy_string if element.isdigit()]
				dy = e_dy_string[0]
				print("dy: ", dy)
				pos = int(dy)
			else:
