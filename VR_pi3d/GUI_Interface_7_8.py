...
def default():
    ...
    ImageName     = [os.path.splitext(s)[0] for s in file_list]
...
def vr_image_build(file_list, corridor_pattern_dir_list, size):
    ...
    for element in corridor_pattern_dir_list:
        if os.path.splitext(element)[1] == ".jpeg" or os.path.splitext(element)[1] == ".png":
            path_c = corridor_pattern_dir + element
            path_n = corridor_pattern_dir + os.path.splitext(element)[0] + ".jpg"
            pic    = Image.open(path_c)
            pic.save(path_n)

            os.remove(path_c)

            #print("vr_build jpeg: ", path_n)
