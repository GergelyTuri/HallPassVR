...
# 1 - onClick_Add          DONE
# 2 - onClick_Delete       DONE
# 3 - onClick_Delete_All   DONE
# 4 - onClick_Generate     DONE
# 5 - onClick_Exit         DONE
# 6 - onClick_upload       DONE
# 7 - onClick_Delete_hi    DONE
# 8 - onClick_Del_hi_all   DONE
# 9 - onClick_Start        
#10 - change combination   DONE
...
def fc_gen():
    ...
    print("fc_gen  output path: ", output_path_12_num)

    # --- this is to store history list ---

    uptohistory(pattern_list_array)

    print("fc_gen  history array: ", history_array)


def fc_upload():
    global history_array

    array_length       = len(history_array)
    randicursel        = np.random.randint(0,array_length)

    writefile2.write(str(randicursel))
    writefile2.write('\n')

    cursel_string      = history_array[randicursel]
    cursel_string2list = cursel_string.split()
    cursel_string2list = [int(element) for element in cursel_string2list]

    print("fc_upload  cursel list: ", cursel_string2list)
...
def fc_delete_hi():
    ...
    delete_comb = history_array[randicursel]
    history_array.remove(delete_comb)

    print("fc_delete_hi  history array: ", history_array)


def fc_delete_hi_all():
    global history_array

    history_array.clear()

    print("fc_delete_hi_all  history array: ", history_array)



def uptohistory(inputarray):
    global history_array

    inputarray2list = [str(int) for int in inputarray]

    inputarray2string = ' '.join(inputarray2list)

    history_array.append(inputarray2string)
