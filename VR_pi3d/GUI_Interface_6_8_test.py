...
def default():
    ...
    output_path_12_num = [10000 for i in range(12)]
...
def history_comb_read():
    ...
    print("FUNCTION: HISTORY COMBINATION READ \n")
...
#---------------------- onClick_Add ------------------------------------
# Read curser selection number, ADD
# corrsponding pattern name (black/red etc) in ImageName list to Listbox
# corrsponding PathToPattern (image/....) to curr_list list
# corrsponding ImageNumCount(1,2,3,4) to curr_list_count  
# ----------------------------------------------------------------------
def onClick_Add():
    print("FUNCTION: ONCLICK_ADD FUNCTION \n")
    global upload_sig
    global curr_list
    global curr_list_count
    global limit_gen_patt 

    comb_size_gen = PatternList.size()
    pattern_gen = np.random.randint(0, comb_size_gen)

    file   = pathToImages[pattern_gen]
    canvas2 = Canvas(ListFrame, width = 40, height = 40)
    canvas2.grid(row = 5, column = 1, columnspan = 1, sticky = W)

    img_resize_canvas(file, size_pattern, canvas2)

    Add_pattern = ImageName[pattern_gen]

    Comb_size = len(curr_list)
    if upload_sig == 1:
        #messagebox.showerror("Error", "Combination exist")
        print("onClick_Add/Error: Combination Exist")
    elif upload_sig == 0:
        if Comb_size < limit_gen_patt:

            CombList.insert(END, Add_pattern)
            curr_list.append(pathToImages[pattern_gen])
            curr_list_count.append(ImageNumCount[pattern_gen])
            Pattern2Path.append(pathToImages[pattern_gen])
        else:
            #messagebox.showerror("Error", "Out of Limit")
            print("onClick_Add/Error: Out of Limit")


    print("onClick_Add: ", curr_list_count)

# ----------------------- onClick_Delete -------------------------------
# Read curser selection value, DELETE
# corrsponding pattern name from listbox using ANCHOR
# corrspodning path2pattern from curr_list
# corrsponding delete_pattern_count from curr_list_count
# ----------------------------------------------------------------------
def onClick_Delete():
    print("FUNCTION: ONCLICK_DELETE \n")
    global curr_list
    global curr_list_count
    global upload_sig
    ...
    if CombList.size() == 0:
        pass
        print("for test, the list is empty")

    else:
        if upload_sig == 0:
            #print("current list is :", curr_list)

            array_length = CombList.size()

            print("onClick_Delete/CombList-size: ", array_length)

            Delete_pattern_count_rand = np.random.randint(0, array_length)
            
            print("onClick_Delete/Delete_pattern_count: ",Delete_pattern_count_rand)

            print("onClick_Delete/curr_list: ",curr_list)
            print("onClick_Delete/curr_list_count: ", curr_list_count)

            Delete_pattern       = curr_list[Delete_pattern_count_rand]
            Delete_pattern_count = curr_list_count[Delete_pattern_count_rand]

            #print("delete botton, the iamge name: ", com_test[int(CombList.curselection()[0])])
            #CombList.delete(ANCHOR)
            CombList.delete(Delete_pattern_count_rand)
            curr_list.remove(Delete_pattern)
            Pattern2Path.remove(Delete_pattern)

            curr_list_count.remove(Delete_pattern_count)

            print("onClick_Delete: ", curr_list_count)
            print("onClick_Delete: ",curr_list)
            print("onClick_Delete: ",CombList.get(0, END))
...
def onClick_Delete_All():
    ...
    print("FUNCTION: ONCLICK_DELETE_ALL")
    ...
    print("onClick_Delete_all: ", curr_list_count)
...
# --------------------------onClick_Generate-------------------------------
# check upload signal and decide generating pattern or path -> decide limit
# give error is nothing in list, give warning if pattern is not enough 
# check which combination status is and generate signals used for canvas 
# generate function and grounps of indexs to replace in output list
# -------------------------------------------------------------------------


def onClick_Generate():

    print("FUNCTION: ONCLICK_GENERATE \n")
    ...
    if GenArry_size == 0:
        ...
        print("onClick_generation/Error: Empty pattern")
    elif GenArry_size < limit:
        #messagebox.showwarning("Warning", "Need "+str(limit)+" Patterns")
        print("onClick_generation/Warning: Need Pattern")
    else:
        ...
        if upload_sig == 0:
            ...
            print("generate/UPLOAD FROM PATTERN: ", comb_list_count)
            print('\n')
            ...
            if history_add_enable == 0:
                ...
                print("onClick_generation/Error, Combination already exist")
        ...
        if upload_sig == 1:
            ...
            print("generate/UPLOAD FROM HISTORY: ", comb_list_count)
            print('\n')
        ...
        print("onClick_generation/Information: Generate Complete")
...
# ---------------------------------------------------- onClikc_upload() ---------------------------------------
# Read history_comb_pos (1,2,3, etc....) using curser. Find history combination path from 
# HistoryList. Read pattern count list from readfile. UPLOAD them to upload_name
# Same as the combination name
# -------------------------------------------------------------------------------------------------------------

def onClick_upload():
    print("FUNCTION: ONCLICK_UPLOAD \n")
    ...
    array_length = HistoryList.size()

    if array_length == 0:
        pass 
    else:
        history_comb_pos         = np.random.randint(0, array_length)
        Upload_history_comb_path = path2historyComb[history_comb_pos]
        ...
        comb_name   = HistoryList.get(history_comb_pos)
        ...
        if CombList.size() < limit_gen_comb:
            ...
            #messagebox.showerror("Error", "Out of Limit")
            print("onClick_upload/Error: Out of Limit ")

        print("onclick_upload: ", comb_name, str(upload_list))


def onClick_Delete_hi():
    print("FUNCTION: ONCLICK_DELETE_HI \n")
    ...
    if array_length == 0:
        ...
        click_count = np.random.randint(0, array_length)
        comb_name   = HistoryList.get(click_count)
        #print(comb_name)
        delete_hi(comb_name)
        history_comb_read()
...
def onClick_Del_hi_all():
    print("FUNCTION: ONCLICK_DEL_HI_ALL \n")
    ...
    if result=='yes':
        ...
        print("onClick_Del_hi_all/Information: Delete All History finished")
    ...
    while True: 
        ...
        if line == "":
            ...
            if compare_last_digt == click_line_mark:
                ...
                print("delete_hi : ", deletestring, compare_last_digt)
                pass  
            else:
...
def HistoryPath(sentence):
    ...
    if result_name == '' or result_length_digit == '':
        ...
        print("HistoryPath/Error :Path Name and Length is not defined")

    else:
        history_path_img    = history_path_folder + result_name + '.jpg'
        ...
        print("HistoryPath /Information,Information saved")
...
def fc_comb_select():
    ...
    if   (comb_select == lab_comb1):
        currStr = 1
    elif (comb_select == lab_comb2):
        currStr = 2
    elif (comb_select == lab_comb3):
        currStr = 3
...
def test_engine():
    ...
    #operation_num = np.random.randint(1,30)
    #print("test_engine: ", operation_num)
    count = 0

    command_array = np.random.randint(1,9, 2000)

    for element in command_array:
        time.sleep(0.5)

        print('\n')
        print("test_engine/curr_list_count: ", curr_list_count)
        print("test_engine/curr_list: ", curr_list)
        print("test_engine/CombList: ",CombList.get(0,END))
        print("test_engine/currStr: ", currStr)
        print("test_engine/output_path_12_num: ", output_path_12_num)
        print("test_engine/output_path_12: ", output_path_12)
        print("test_engine/round count: ", count )
...
Button_Start      = Button(root,        text = "Start",      width = 10, height = 2, command = test)
...
test = Thread(target = test_engine)
